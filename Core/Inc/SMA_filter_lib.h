...
/* Макросы
 * -------------------------------------------------------------------------------------------------------------------------------*/
#define SMA_FILTER_ORDER 10 /* Порядок SMA фильтра */

/* Выберите версию фильтра(Делают они одно и то же, просто разными способами.
 * Какой из них оптимальнее - решать Вам)*/
...
uint16_t SMA_FILTER_Get_Value(uint8_t *SMA_Filter_counter,
                              uint16_t *SMA_Filter_buffer,
                              uint32_t *SMA_Filter_Result, uint16_t *RAW_Data);
#endif
